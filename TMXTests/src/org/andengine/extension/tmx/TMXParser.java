...
public class TMXParser extends DefaultHandler implements TMXConstants {
	...
	public TMXParser(final Context pContext, final TextureManager pTextureManager, final TextureOptions pTextureOptions, final ITMXTilePropertiesListener pTMXTilePropertyListener, final String pAssetBasePath) {
		...
		if(pLocalName.equals(TMXConstants.TAG_MAP)){
			...
			if(tsxTileSetSource == null) {
				...
				try {
					...
					tmxTileSet = new TSXLoader(this.mContext, this.mTextureManager, this.mTextureOptions, tsxDirPath).loadFromAsset(this.mContext, firstGlobalTileID, mAssetBasePath + tsxTileSetSource);
			...
			tmxTileSets.get(tmxTileSets.size() - 1).setImageSource(this.mContext, this.mTextureManager, pAttributes, mAssetBasePath);
